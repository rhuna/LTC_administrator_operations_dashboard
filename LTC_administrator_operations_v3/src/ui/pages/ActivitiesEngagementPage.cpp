#include "ActivitiesEngagementPage.h"
...
ActivitiesEngagementPage::ActivitiesEngagementPage(DatabaseManager* db, QWidget* parent)
    : QWidget(parent) {
    auto* root = new QVBoxLayout(this);
    root->setContentsMargins(8, 8, 8, 8);
    root->setSpacing(14);

    auto* heading = new QLabel("Activities / Resident Engagement", this);
    heading->setStyleSheet("font-size: 20px; font-weight: 700;");
    root->addWidget(heading);

    auto* subtitle = new QLabel(
        "Track activity attendance risks, engagement plans, behavioral redirection ideas, volunteer coordination, and resident-preference follow-up in one simple workspace.",
        this);
    ...
    auto refreshTable = [db, tableWidget, cols]() {
        ...
        const auto rows = db->fetchTable("activities_items", cols);
    ...
    focusArea->setPlaceholderText("Engagement / Preference / Attendance");
    auto* itemName = new QLineEdit(this);
    itemName->setPlaceholderText("Activity / engagement item");
    auto* owner = new QLineEdit(this);
    owner->setPlaceholderText("Owner");
    auto* notes = new QLineEdit(this);
    notes->setPlaceholderText("Notes");
    auto* button = new QPushButton("Add Activities Item", this);
    ...
    QObject::connect(button, &QPushButton::clicked, this, [=]() {
        ...
        if (db->addRecord("activities_items", values)) {
            dateEdit->clear();
            residentName->clear();
            focusArea->clear();
            itemName->clear();
