...
MdsTripleCheckPage::MdsTripleCheckPage(DatabaseManager* db, QWidget* parent)
    ...
    auto refreshMdsTable = [db, mdsTable, mdsCols]() {
        ...
        for (const auto& row : rows) {
            ...
            for (int c = 0; c < mdsCols.size(); ++c) mdsTable->setItem(r, c, new QTableWidgetItem(row.value(mdsCols[c])));
        }
    };
    ...
    auto refreshTherapyTable = [db, therapyTable, therapyCols]() {
        ...
        for (const auto& row : db->fetchTable("therapy_items", therapyCols)) {
            ...
            for (int c = 0; c < therapyCols.size(); ++c) therapyTable->setItem(r, c, new QTableWidgetItem(row.value(therapyCols[c])));
        }
    };
    ...
    auto refreshDiagnosisTable = [db, diagnosisTable, diagnosisCols]() {
        ...
        for (const auto& row : rows) {
            ...
            for (int c = 0; c < diagnosisCols.size(); ++c) diagnosisTable->setItem(r, c, new QTableWidgetItem(row.value(diagnosisCols[c])));
        }
    };
    root->addWidget(diagnosisTable);

    QObject::connect(button, &QPushButton::clicked, this, [=]() {
        QMap<QString, QString> values{{"resident_name", residentName->text()}, {"payer", payer->text()}, {"assessment_type", assessmentType->text()}, {"ard_date", ardDate->text().isEmpty() ? "2026-04-23" : ardDate->text()}, {"triple_check_date", tripleCheckDate->text().isEmpty() ? "2026-04-24" : tripleCheckDate->text()}, {"status", "Open"}, {"owner", owner->text().isEmpty() ? "MDS Coordinator" : owner->text()}, {"notes", notes->text()}};
        db->addRecord("mds_items", values);
        refreshMdsTable();
        residentName->clear(); payer->clear(); assessmentType->clear(); ardDate->clear(); tripleCheckDate->clear(); owner->clear(); notes->clear();
    });
    ...
    for (const auto& row : db->fetchTable("managed_care_items", managedCols)) {
        ...
        for (int c = 0; c < managedCols.size(); ++c) managedTable->setItem(r, c, new QTableWidgetItem(row.value(managedCols[c])));
    }
